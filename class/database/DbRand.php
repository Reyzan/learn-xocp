...
/******************************************************************
 ...
 Email: jay@vermonster.com
 Purpose: Provide an OO API for redundant MySQL connection
 
 This software is now distributed under the terms of the 
 Lesser GNU Public License
 
 $Id: DbRand.php,v 1.1.1.1 2002/11/19 19:57:01 adiet9000 Exp $
 
 Usage:
 
 // Connection to MASTER
 $object = new DbRand("10.0.0.1","login","pass","test");
...
class DbRand {
 ...
 var $RANDOMIZE;
   ...
   function DbRand ($master, $login, $pass, $db, $timeout=5, $socket=true) {
   ...
   function db_connect($db) {
       ...
       * We need to see what type of query is being run */
       ...
       if ($fp = fsockopen($db['host'], 3306, &$errno, &$errstr, $this->TIMEOUT)) {
           
           if (!$this->LINK_ID = mysql_connect($db['host'], $db['user'], $db['pass'])) {
    ...
    * query against random db host.*/
      ...
      if (eregi("^select", $query) & $this->RANDOMIZE & $this->SLAVES) {
