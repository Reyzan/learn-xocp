...
if(!defined("C_TEXTSANITIZER_INCLUDED")){
   define("C_TEXTSANITIZER_INCLUDED",1);
...
class MyTextSanitizer {
   ...
   */
   ...
   function &getInstance(){
      if ( isset($GLOBALS['xocpMyts']) && get_class($GLOBALS['xocpMyts']) == "mytextsanitizer" ) {
         return $GLOBALS['xocpMyts'];
      }
   ...
   function smiley($message) {
      ...
      if ( count($this->smileys) == 0 ) {
         if ($getsmiles = $db->query("SELECT * FROM ".PREFIX."smiles")){
            while ($smiles = $db->fetchArray($getsmiles)) {
               $message = str_replace($smiles['code'], "<img src='".XOCP_URL."/images/smilies/".$smiles['smile_url']."' />", $message);
               array_push($this->smileys, $smiles);
            }
         }
      } else {
         foreach ( $this->smileys as $smile ) {
            $message = str_replace($smile['code'], "<img src='".XOCP_URL."/images/smilies/".$smile['smile_url']."' />", $message);
         }
      }
      return $message;
   }
   ...
   function desmile($message) {
      ...
      if ( count($this->smileys) == 0 ) {
         if ($getsmiles = $db->query("SELECT * FROM ".PREFIX."smiles")){
            while ($smiles = $db->fetchArray($getsmiles)) {
               $message = str_replace("<img src='".XOCP_URL."/images/smilies/".$smiles['smile_url']."' />", $smiles['code'], $message);
               array_push($this->smileys, $smiles);
            }
         }
      } else {
         foreach ( $this->smileys as $smile ) {
            $message = str_replace("<img src='".XOCP_URL."/images/smilies/".$smiles['smile_url']."' />", $smile['code'], $message);
         }
      }
      return $message;
   }
